...
void divide_2_4(Board& act_board, int action, bool is_left){
...
Node* GeneralCommonStrategy::six_common_fields(Board& act_board, int action)
{
    ...
    if(act_board.node_type == AND){
        ...
        if(action == 7 || action == 47 || action == 2 || action == 42){
            ...
            divide_2_4(act_board, action, is_left);
        }
        else{
            assert(0);
        }
            ...
            if(score == 1){
                ...
                if(is_inner){
                    ...
                    if(has(center-1, act_board.black) && has(center+1, act_board.black)){
                        ...
                        // move free, but last is common
                        act_board.forbidden_all &= ~act_board.get_valids();
                        return add_or_create(act_board);
                    }
                    else if(has(center-1, act_board.black) || has(center+1, act_board.black)){
                        // move free, but last is common
                        act_board.forbidden_all &= ~act_board.get_valids();
                        return add_or_create(act_board);
                    }
                ...
                if(has(up, act_board.black) && has(down, act_board.black)){
                    //display(act_board, true);
                    //assert(0);
                    // Move free
                    // TODO: implement on other side
                }
                    ...
                    //child3.white |= (1ULL << free);
                    int opposite = (action / ROW) * ROW + 4 - (action % ROW);
                    child3.white &= ~(1ULL << opposite);
                    child3.black |= (1ULL << opposite);
                    node->children[2] = add_or_create(child3);
    ...
    if(num_common_fields == 1){
        ...
        if(act_board.node_type == AND){
            ...
            if(is_inner){
                // TODO: pretty...
                if(is_left){
                    if((1ULL << 0) | act_board.white){
                        if(act_board.is_valid(11)) act_board.move(11, -1);
                        //else act_board.node_type = OR;
                    }
                    else if((1ULL << 10) | act_board.white){
                        if(act_board.is_valid(13)) act_board.move(13, -1);
                        //else act_board.node_type = OR;
                    }
                }
                else{
                    if((1ULL << 45) | act_board.white){
                        if(act_board.is_valid(36)) act_board.move(36, -1);
                        //else act_board.node_type = OR;
                    }
                    else if((1ULL << 35) | act_board.white){
                        if(act_board.is_valid(38)) act_board.move(38, -1);
                        //else act_board.node_type = OR;
                    }
                }
            }
            else{
                deactivate_line(act_board, is_left?0:45);
                deactivate_line(act_board, is_left?10:35);
                act_board.node_type = OR;
            }
